...
-- 2.	Hiển thị thông tin của tất cả nhân viên có trong hệ thống có tên bắt đầu là một trong các ký tự “H”, “T” hoặc “K” và có tối đa 15 kí tự.
 ...
 select *,year(curdate()) - year(ngay_sinh)  as so_tuoi from khach_hang where year(curdate()) - year(ngay_sinh)  between 18 and 50 and (dia_chi like('%Đà Nẵng') or dia_chi like ('%Quảng Trị'));
...
where kh.ma_loai_khach = (select ma_loai_khach from loai_khach  where ten_loai_khach = 'Diamond')
...
select dv.ma_dich_vu,dv.ten_dich_vu,  dv.dien_tich,dv.chi_phi_thue,ldv.ten_loai_dich_vu,dv.dien_tich,dv.so_nguoi_toi_da from dich_vu dv
left join loai_dich_vu ldv on ldv.ma_loai_dich_vu=dv.ma_loai_dich_vu
where  dv.ma_dich_vu in(
select ma_dich_vu from  hop_dong hd
...
FROM khach_hang;
 ...
 select month(ngay_lam_hop_dong) as 'Thang', count(ma_hop_dong) as'so_lan_dat' from hop_dong where year(ngay_lam_hop_dong)=2021
...
left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong = hd.ma_hop_dong
left join dich_vu_di_kem dvdk on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
where year(hd.ngay_lam_hop_dong) = 2020 and quarter(hd.ngay_lam_hop_dong) = 4 and hd.ngay_lam_hop_dong not in (
select ngay_lam_hop_dong from hop_dong where year(ngay_lam_hop_dong) = 2021 and quarter(ngay_lam_hop_dong) in (1,2)
)
group by kh.ma_khach_hang,hd.ma_hop_dong;
...
-- de cho viec back up du lieu thi k nen xoa database ma nen them 1 cot status vs 2 gia tri 1  hoac 0)
